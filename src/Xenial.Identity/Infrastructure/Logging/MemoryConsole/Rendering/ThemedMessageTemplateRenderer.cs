...
namespace Xenial.Identity.Infrastructure.Logging.MemoryConsole.Rendering;
...
internal class ThemedMessageTemplateRenderer
{
    ...
    public ThemedMessageTemplateRenderer(ConsoleTheme theme, ThemedValueFormatter valueFormatter, bool isLiteral)
    {
        ...
        unthemedValueFormatter = valueFormatter.SwitchTheme(noTheme);
    }
    ...
    {
        var count = 0;
        using (theme.Apply(output, ConsoleThemeStyle.Text, ref count))
        {
    ...
    private int RenderPropertyToken(PropertyToken pt, IReadOnlyDictionary<string, LogEventPropertyValue> properties, TextWriter output)
    {
        ...
        {
            var count = 0;
            using (theme.Apply(output, ConsoleThemeStyle.Invalid, ref count))
            {
        ...
        {
            return RenderValue(theme, valueFormatter, propertyValue, output, pt.Format);
        }
        ...
        var invisibleCount = RenderValue(theme, valueFormatter, propertyValue, valueOutput, pt.Format);
    ...
    private int RenderAlignedPropertyTokenUnbuffered(PropertyToken pt, TextWriter output, LogEventPropertyValue propertyValue)
    {
        ...
        var valueOutput = new StringWriter();
        _ = RenderValue(noTheme, unthemedValueFormatter, propertyValue, valueOutput, pt.Format);
        ...
        {
            var invisible = RenderValue(theme, valueFormatter, propertyValue, output, pt.Format);
    ...
    private int RenderValue(ConsoleTheme theme, ThemedValueFormatter valueFormatter, LogEventPropertyValue propertyValue, TextWriter output, string? format)
    {
        ...
        return valueFormatter.Format(propertyValue, output, format, isLiteral);
    }
}
