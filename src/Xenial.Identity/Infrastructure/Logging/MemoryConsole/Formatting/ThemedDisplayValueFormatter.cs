...
internal class ThemedDisplayValueFormatter : ThemedValueFormatter
{
    ...
    public ThemedDisplayValueFormatter(ConsoleTheme theme, IFormatProvider? formatProvider)
        : base(theme) => this.formatProvider = formatProvider;

    public override ThemedValueFormatter SwitchTheme(ConsoleTheme theme) => new ThemedDisplayValueFormatter(theme, formatProvider);
    ...
    protected override int VisitSequenceValue(ThemedValueFormatterState state, SequenceValue sequence)
    {
        ...
        for (var index = 0; index < sequence.Elements.Count; ++index)
        {
            ...
            delim = ", ";
            _ = Visit(state, sequence.Elements[index]);
        }
    ...
    public int FormatLiteralValue(ScalarValue scalar, TextWriter output, string? format)
    {
        ...
        {
            if (value is int or uint or long or ulong or
                decimal or byte or sbyte or short or
                ushort or float or double)
            {
