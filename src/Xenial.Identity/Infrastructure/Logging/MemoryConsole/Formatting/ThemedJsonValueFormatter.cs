...
namespace Xenial.Identity.Infrastructure.Logging.MemoryConsole.Formatting
{
    internal class ThemedJsonValueFormatter : ThemedValueFormatter
    {
        ...
        public ThemedJsonValueFormatter(ConsoleTheme theme, IFormatProvider? formatProvider)
            : base(theme)
        {
            displayFormatter = new ThemedDisplayValueFormatter(theme, formatProvider);
            this.formatProvider = formatProvider;
        }

        public override ThemedValueFormatter SwitchTheme(ConsoleTheme theme) => new ThemedJsonValueFormatter(theme, formatProvider);
        ...
        protected override int VisitScalarValue(ThemedValueFormatterState state, ScalarValue scalar)
        {
            ...
            return state.IsTopLevel
                ? displayFormatter.FormatLiteralValue(scalar, state.Output, state.Format)
        ...
        protected override int VisitSequenceValue(ThemedValueFormatterState state, SequenceValue sequence)
        {
            ...
            for (var index = 0; index < sequence.Elements.Count; ++index)
            {
                ...
                delim = ", ";
                _ = Visit(state.Nest(), sequence.Elements[index]);
            }
